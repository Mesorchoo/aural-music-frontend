...
<script setup>
import { reactive, computed, inject, onBeforeMount, onBeforeUnmount, onActivated } from 'vue'

// Named so HomeView's <KeepAlive include="ArtistsView"> keeps this page alive
...
// Shown again from the KeepAlive cache: put the scroll back and refresh the list
onActivated(() => {
    if (!state.loaded) return // first mount; onBeforeMount already requested the list
    restoreScroll()
    navigator.serviceWorker.ready.then( registration => {
        registration.active.postMessage({
            action: 'get_artists'
